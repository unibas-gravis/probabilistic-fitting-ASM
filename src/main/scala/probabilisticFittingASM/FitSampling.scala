...
import probabilisticFittingASM.sampling.evaluators.{IndependentASMEvaluator, ModelPriorEvaluator, VolumePrior}
import probabilisticFittingASM.sampling.parameters.{ModelFittingParameters, PoseParameters, ScaleParameter, ShapeParameters}
import probabilisticFittingASM.sampling.proposals.{ShapeProposal, PoseProposal}
...
object FitSampling {
  ...
  final val fittingMethod = DataProvider.fittingUsingSampling
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dataProvider = new DataProvider(opts.root(),opts.leaveOneOut())

    if (opts.leaveOneOut()) {
      for (i <- (0 until dataProvider.ids.size) /*.par*/ ) {
        val trainingData = dataProvider.ids.patch(i, Nil, 1)
        val testDatum = dataProvider.ids(i)
        println(s"fitting - $testDatum")
  ...
  def fitModel(data: DataProvider, testDatum: String, asm: ActiveShapeModel, debug: Boolean = false)(implicit rng: Random): Unit = {
    ...
    val samplingMesh = fit(targetImage,asm,debug = debug)
  ...
  def fit(target: DiscreteScalarImage[_3D,Float], asm: ActiveShapeModel, iterations: Int = 10000, debug: Boolean = false)(implicit rng: Random): TriangleMesh[_3D] = {
    ...
    val filtered = MetropolisFilterProposal(generator,ProductEvaluator(ModelPriorEvaluator(asm.statisticalModel),VolumePrior(target,asm)))
    val chain = MetropolisHastings(filtered, appearanceEval)
    ...
    val bestLogger = BestSampleLogger(ProductEvaluator(appearanceEval,ModelPriorEvaluator(asm.statisticalModel),VolumePrior(target,asm)))
